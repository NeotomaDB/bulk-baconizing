#' @title Make core file for Bacon runs.
#' @description
#' @param x A chronology from the Neotoma API
#' @param settings A \code{data.frame} generated from a \code{yaml} settings file.
#' @param core_param A one row \code{data.frame} representing the parameters for the dataset.
#' @param core_top The age of the core top.

make_coredf <- function(x,
  settings,
  core_param,
  core_top = NA) {
    ...
    build_row <- function(z) {
      ...
      if (is.null(unlist(z$geochron))) {
        ...
        if (z$geochron$geochrontype == "Lead-210" &
          ...
          age_err <- z$geochron$age - as.numeric(core_param$core_top)
