...
#' @param add Add a value to the maximum hiatus length if a boundary is chosen. Defaults to \code{add=0}. Can be used if Bacon complains that the parameters are out of support.
...
Bacon <- function(core="MSB2K", thick=5, coredir="", prob=0.95, d.min=NA, d.max=NA, d.by=1, depths.file=FALSE, depths=c(), unit="cm", acc.shape=1.5, acc.mean=20, mem.strength=4, mem.mean=0.7, boundary=NA, hiatus.depths=NA, hiatus.max=10000, add=0, after=.000001, cc=1, cc1="IntCal13", cc2="Marine13", cc3="SHCal13", cc4="ConstCal", ccdir="", postbomb=0, delta.R=0, delta.STD=0, t.a=3, t.b=4, normal=FALSE, suggest=TRUE, reswarn=c(10,200), remember=TRUE, ask=TRUE, run=TRUE, defaults="default_settings.txt", sep=",", dec=".", runname="", slump=c(), BCAD=FALSE, ssize=2000, th0=c(), burnin=min(200, ssize), MinYr=c(), MaxYr=c(), cutoff=.001, plot.pdf=TRUE, dark=1, date.res=100, yr.res=200, ...) {
  ...
  if(ccdir == "")
    ccdir <- paste(system.file("extdata", package="rbacon"), "/Curves/", sep="")
  ...
  if(ncol(dets) > 4 && length(cc) > 0) {
    ...
    if(suggest) { # adapt prior for mean accumulation rate?
      ...
      ballpacc <- ballpacc[ballpacc > 0] # don't suggest 0
  ...
  if(length(slump) > 0) {
    ...
    if(length(info$slumpboundary) > 0) {
  ...
  if(!is.na(boundary)[1]) {
    ...
    info$hiatus.max <- add+info$acc.mean
  }
  .assign_to_global("info", info)
  ...
  if (length(showConnections()) > 0) {
    cat(paste0("Existing connections: ", showConnections()))
  }
...
agedepth <- function(set = set, BCAD=set$BCAD, unit="cm", d.lab=c(), yr.lab=c(), d.min=set$d.min, d.max=set$d.max, d.by=set$d.by, depths=set$depths, depths.file=FALSE, yr.min=c(), yr.max=c(), dark=c(), prob=set$prob, rounded=0, d.res=400, yr.res=400, date.res=100, grey.res=100, rotate.axes=FALSE, rev.yr=FALSE, rev.d=FALSE, maxcalc=500, height=15, mirror=TRUE, up=TRUE, cutoff=.001, plot.range=TRUE, panels=layout(1), range.col=grey(.5), range.lty="12", mn.col="red", mn.lty="12", med.col=NA, med.lty="12", C14.col=rgb(0,0,1,.35), C14.border=rgb(0,0,1,.5), cal.col=rgb(0,.5,.5,.35), cal.border=rgb(0,.5,.5,.5), hiatus.col=grey(0.5), hiatus.lty="12", greyscale=grey(seq(1, 0, length=grey.res)), slump.col=grey(0.8), normalise.dists=TRUE, cc=set$cc, title=set$core, title.location="topleft", after=set$after, bty="l", mar=c(3,3,1,1), mgp=c(1.5,.7,.0), xaxs="r", yaxs="r", xaxt="s", yaxt="s", plot.pdf=FALSE, dates.only=FALSE, model.only=FALSE, talk=FALSE) {
    ...
    panels else { # layout(1) can mess things up if plotting within an existing panel...
      ...
      if(!is.na(set$hiatus.depths)[1])
        if(is.na(set$boundary)[1])
          pn <- c(1:4, rep(5,4))
      layout(matrix(pn, nrow=2, byrow=TRUE), heights=c(.3,.7))
      .PlotLogPost(set = set, 0, set$Tr) # convergence information
      .PlotAccPost(set = set)
      .PlotMemPost(set = set, set$core, set$K, "", set$mem.strength, set$mem.mean, ds=1, thick=set$thick)
      if(!is.na(set$hiatus.depths[1]))
        if(is.na(set$boundary)[1])
  ...
  calib.plot(set, BCAD=BCAD, rotate.axes=rotate.axes, height=height, mirror=mirror, up=up, date.res=date.res, cutoff=cutoff, C14.col=C14.col, C14.border=C14.border, cal.col=cal.col, cal.border=cal.border, new.plot=FALSE, normalise.dists=normalise.dists)
...
#' \url{https://projecteuclid.org/download/pdf_1/euclid.ba/1339616472}
#' @export
thinner <- function(proportion=0.1, set) {
  output <- read.table(paste(set$prefix, ".out", sep=""))
  if(proportion >= 1)
    stop("\nCannot remove that many iterations, there would be none left!\n\n", call.=FALSE)
...
#' @export
Baconvergence <- function(core="MSB2K", runs=5, suggest=FALSE, set, ...) {
  MCMC <- list()
...
#' @param BCAD The calendar scale of graphs and age output-files is in \code{cal BP} by default, but can be changed to BC/AD using \code{BCAD=TRUE}.
#' @param slumpdepths An internal variable to deal with cores with slumps
#' @author Maarten Blaauw, J. Andres Christen
...
#'  \url{https://projecteuclid.org/download/pdf_1/euclid.ba/1339616472}
#' @export
Bacon.Age.d <- function(d, set, its=set$output, BCAD=set$BCAD, slumpdepths=c()) {
  if(length(d) > 1)
...
#' \url{https://projecteuclid.org/download/pdf_1/euclid.ba/1339616472}
#' @export
Bacon.hist <- function(d, set, BCAD=set$BCAD, yr.lab=c(), yr.lim=c(), hist.lab="Frequency", calc.range=TRUE, hist.lim=c(), draw=TRUE, prob=set$prob, hist.col=grey(0.5), hist.border=grey(.2), range.col="blue", med.col="green", mean.col="red") {
...
Bacon.rng <- function(d, set, BCAD=set$BCAD, prob=set$prob) {
  outfile <- paste(set$prefix, ".out", sep="")
  if(length(set$output) == 0 || length(set$Tr) == 0) {
    set <- .Bacon.AnaOut(outfile, set)
    .assign_to_global("set", set)
...
#' \url{https://projecteuclid.org/download/pdf_1/euclid.ba/1339616472}
#' @export
proxy.ghost <- function(proxy=1, proxy.lab=c(), proxy.res=250, yr.res=250, grey.res=100, set, dark=1, darkest=1, rotate.axes=FALSE, proxy.rev=FALSE, yr.rev=FALSE, plot.mean=FALSE, mean.col="red", yr.lim=c(), proxy.lim=c(), sep=",", xaxs="i", yaxs="i", xaxt="s", yaxt="s", bty="l", BCAD=set$BCAD, yr.lab=ifelse(BCAD, "BC/AD", "cal yr BP")) {
  if(length(set$Tr)==0)
...
#' \url{https://projecteuclid.org/download/pdf_1/euclid.ba/1339616472}
#' @export
accrate.depth <- function(d, set, cmyr=FALSE) {
  if(min(set$d) <= d && max(set$d) >= d)
    accs <- set$output[,1+max(which(set$d <= d))] else
      accs <- NA
...
#'  \url{https://projecteuclid.org/download/pdf_1/euclid.ba/1339616472}
#' @export
accrate.age <- function(age, set, cmyr=FALSE, BCAD=set$BCAD) {
...
#' \url{https://projecteuclid.org/download/pdf_1/euclid.ba/1339616472}
#' @export
accrate.depth.ghost <- function(set, d=set$d, d.lim=c(), acc.lim=c(), d.lab=c(), cmyr=FALSE, acc.lab=c(), dark=1, grey.res=100, prob=0.95, plot.range=TRUE, range.col=grey(0.5), range.lty=2, plot.mean=TRUE, mean.col="red", mean.lty=2, rotate.axes=FALSE, rev.d=FALSE, rev.acc=FALSE) {
...
#' \url{https://projecteuclid.org/download/pdf_1/euclid.ba/1339616472}
#' @export
accrate.age.ghost <- function(set, yr.lim=c(), yr.lab=c(), yr.res=200, grey.res=50, prob=.95, plot.range=TRUE, range.col=grey(0.5), range.lty=2, plot.mean=TRUE, mean.col="red", mean.lty=2, acc.lim=c(), acc.lab=c(), upper=0.99, dark=50, BCAD=set$BCAD, cmyr=FALSE, rotate.axes=FALSE, rev.yr=FALSE, rev.acc=FALSE, xaxs="i", yaxs="i", bty="o") {
  if(length(yr.lim) == 0) {
...
#' \url{https://projecteuclid.org/download/pdf_1/euclid.ba/1339616472}
#' @export
flux.age.ghost <- function(proxy=1, yr.lim=c(), yr.res=200, set, flux=c(), plot.range=TRUE, prob=.8, range.col=grey(0.5), range.lty=2, plot.mean=TRUE, mean.col="red", mean.lty=2, flux.lim=c(), flux.lab="flux", upper=.95, dark=set$dark, BCAD=set$BCAD, yr.lab=c(), rotate.axes=FALSE, rev.flux=FALSE, rev.yr=FALSE) {
...
#' \url{https://projecteuclid.org/download/pdf_1/euclid.ba/1339616472}
#' @export
AgesOfEvents <- function(window, move, set, plot.steps=FALSE, BCAD=set$BCAD, yr.lab=c(), yr.lim=c(), prob.lab="probability", prob.lim=c(), rotate.axes=FALSE, rev.yr=TRUE, yaxs="i", bty="l") {
  if(move == 0)
...
.agedepth.ghost <- function(set, d.min=set$d.min, d.max=set$d.max, BCAD=set$BCAD, rotate.axes=FALSE, d.res=400, yr.res=400, grey.res=100, dark=c(), colours=grey(seq(1, 0, length=100)), xaxt="s", yaxt="s", yr.lim) {
...
.bacon.calib <- function(dat, set, date.res=100, normal=set$normal, t.a=set$t.a, t.b=set$t.b, delta.R=set$delta.R, delta.STD=set$delta.STD, ccdir="") {
  # read in the curves
...
.PlotAccPrior <- function(s, mn, set, main="", xlim=c(0, 3*max(mn)), xlab=paste("Acc. rate (yr/", noquote(set$unit), ")", sep=""), ylab="Density", add=FALSE, legend=TRUE, cex=.9) {
...
.PlotMemPrior <- function(s, mn, thick, ds=1, set, xlab="Memory (ratio)", ylab="Density", main="", add=FALSE, legend=TRUE, cex=.9) {
  o <- order(s, decreasing=TRUE)
  priors <- unique(cbind(s[o],mn[o])[,1:2])
  x <- 0
...
.PlotHiatusPrior <- function(mx=set$hiatus.max, hiatus=set$hiatus.depths, set, xlab="Hiatus size (yr)", ylab="Density", main="", xlim=c(0, 1.1*max(mx)), add=FALSE, legend=TRUE) {
  if(add)
...
.PlotAccPost <- function(set, s=set$acc.shape, mn=set$acc.mean, main="", xlab=paste("Acc. rate (yr/", set$unit, ")", sep=""), ylab="Frequency") {
  hi <- 2:(set$K-1)
  if(!is.na(set$hiatus.depths)[1])
    for(i in set$hiatus.depths)
...
.PlotMemPost <- function(set, corenam, K, main="", s=set$mem.strength, mn=set$mem.mean, xlab=paste("Memory"), ylab="Density", ds=1, thick) {
  ...
  plot(0, type="n", xlab=xlab, xlim=c(0,1), ylim=c(0, 1.05*max.y), ylab="", main="")
  polygon(post, col=grey(.8), border=grey(.4))
  .PlotMemPrior(s, mn, thick, add=TRUE, xlab="", set=set, ylab=ylab, main=main)
}



# plot the posterior (and prior) of the hiatus
.PlotHiatusPost <- function(set, mx=set$hiatus.max, main="", xlim=c(), xlab=paste("Hiatus size (yr)", sep=""), ylab="Frequency", minbreaks=10, after=set$after) {
  gaps <- c()
  for(i in set$hiatus.depths) {
...
calib.plot <- function(set, BCAD=set$BCAD, rotate.axes=FALSE, rev.d=FALSE, rev.yr=FALSE, yr.lim=c(), date.res=100, d.lab=c(), yr.lab=c(), height=15, mirror=TRUE, up=TRUE, cutoff=.001, C14.col=rgb(0,0,1,.5), C14.border=rgb(0,0,1,.75), cal.col=rgb(0,.5,.5,.5), cal.border=rgb(0,.5,.5,.75), slump.col=grey(0.8), new.plot=TRUE, plot.dists=TRUE, normalise.dists=TRUE) {
...
assign_coredir <- function(coredir, core, ask=TRUE) {
  ...
  cat(paste0("The run's files will be put in this folder: ", coredir, core, "\n\n", collapse = ""))
  return(coredir)
}
...
overlap <- function(set, digits=0) {
  ...
  for(i in 1:length(d)) {
    ...
    age <- quantile(Bacon.Age.d(set = set, d[i]), c((1-set$prob)/2, 1-(1-set$prob)/2))
...
.read.dets <- function(core, coredir, set, sep=",", dec=".", cc=1) {
...
.write.Bacon.file <- function(set) {
  ...
  if(set$d.min < min(dets[,4])) {
    extrap <- c(NA, min(dets[,2]), max(1e4, 100*dets[,3]), set$d.min, 0)
    dets <- rbind(extrap, dets, deparse.level=0)
  }
  if(set$d.max > max(dets[,4])) {
    extrap <- c(NA, max(dets[,2]), max(1e4, 100*dets[,3]), set$d.max, 0)
...
#        set$hiatus.shape <- rep(set$hiatus.shape, length(set$hiatus.depths))
    ...
    for(i in length(hiatus.depths):1)
      cat("\nHiatus ", i-1, ":  ", hiatus.depths[i], ",  ", set$acc.shape[i+1],
        ",  ", set$acc.shape[i+1]/set$acc.mean[i+1], ",  ", NA,
...
.Bacon.AnaOut <- function(fnam, set) {
  out <- read.table(fnam)
  n <- ncol(out)-1
  set$n <- n
  set$Tr <- nrow(out)
