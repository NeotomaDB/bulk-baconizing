...
bacon_age_posts <- function(handle)
{
  ...
  for (i in 1:length(out_files)) {
    ...
    outer <- readr::read_delim(out_files[i], col_names = FALSE,delim = ' ') %>% as.data.frame

    # We can do this match because we know how Bacon writes out files.
    sections <- stringr::str_match(out_files[i], '(?:_)([0-9]*)\\.')[2] %>%
      as.numeric()
    ...
    if (ncol(outer) == (sections + 3)) {
      ...
      for(j in 1:nrow(outer)) {
        ...
        y <- c(outer[j,1], outer[j,1] + cumsum((diff(x) * outer[j,2:(ncol(outer) - 2)]) %>% as.numeric))
        priors[,j] <- approx(x, y, xout = depth %>% unlist())$y
      }
      readr::write_csv(paste0('Cores/', handle, '/', handle, '_', sections, '_priorout.csv'), x = as.data.frame(priors))
    }
