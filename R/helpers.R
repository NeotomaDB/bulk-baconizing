...
{ 
  ...
  depth <- readr::read_csv(paste0('Cores/', handle, '/', handle, '_depths.txt'), col_names = FALSE)
  ...
  for (i in 1:length(out_files)) {
    outer <- readr::read_delim(out_files[i], col_names = FALSE,delim = ' ')
    sections <- stringr::str_extract(out_files[i], '[0-9]*')
    x <- seq(bacon_settings[1,1], bacon_settings[2,1], length.out = ) 
  }
  readr::read_table()
...
call_bacon <- function(site.params){
